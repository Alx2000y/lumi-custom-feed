...
login=$(cut -f1 -d: $mdl/manmod/aunt.cyber | sed "s/@/%40/g")
pass=$(cut -f2 -d: $mdl/manmod/aunt.cyber | sed "s/@/%40/g")
...
`sed -i 3i\ '<a href=/cgi-bin/modules/'$mod'/index.cgi><div class=left>'$title'</div></a>' /www/menu.html`
...
echo "<br><b>Категрия: $categ </b><a href=$SCRIPT_NAME>выбор категории</a>"
elif [ $QUERY_STRING == "searchmod" ] ; then
echo "<br><h2>Результат поиска: </h2><a href=$SCRIPT_NAME>выбор категрии</a>"
fi
echo "<table border=0 width=100%><tr><td>
<table border=0 cellspacing=10><tr><td width=130px>Доступно: `df -h | awk '{print $4}' | sed 's/G//' | sed '2!d'`
</td><td><a style=\"cursor:pointer;\" onclick=\"show('cat1')\">Модуль по умолчанию</a></td>
<td><a style=\"cursor:pointer;\" onclick=\"show('cat2')\">Авторизация</a></td>
<td><a style=\"cursor:pointer;\" onclick=\"show('cat3')\">Поиск</a></td>
